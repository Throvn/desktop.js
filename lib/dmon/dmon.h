...
#define _DMON_LOG_ERRORF(str, ...)                    \
    do                                                \
    {                                                 \
        char msg[512];                                \
        snprintf(msg, sizeof(msg), str, __VA_ARGS__); \
        DMON_LOG_ERROR(msg);                          \
    } while (0);
#endif

#ifndef _DMON_LOG_DEBUGF
#define _DMON_LOG_DEBUGF(str, ...)                    \
    do                                                \
    {                                                 \
        char msg[512];                                \
        snprintf(msg, sizeof(msg), str, __VA_ARGS__); \
...
// IOCP
#ifdef UNICODE
#define _DMON_WINAPI_STR(name, size) \
    wchar_t _##name[size];           \
    MultiByteToWideChar(CP_UTF8, 0, name, -1, _##name, size)
#else
        ...
        if (ev->mask & IN_MODIFY)
        {
            ...
            for (j = i + 1; j < c; j++)
            {
                ...
                else if ((ev->mask & IN_ISDIR) && (check_ev->mask & (IN_ISDIR | IN_MODIFY)))
                {
                    ...
                    if (ev->filepath[l1 - 1] == '/')
                        ev->filepath[l1 - 1] = '\0';
                    if (check_ev->filepath[l2 - 1] == '/')
                        check_ev->filepath[l2 - 1] = '\0';
